...
from connect_four import dropPiece, get_NextOpenRow, printBoard, winningMove
...
def score_position(board, piece):
	...
	## Score center column
	center_array = [int(i) for i in list(board[:, COLUMN_COUNT//2])]#3 all values in column 3
...
def is_terminal_node(board):
	return winning_move(board, PLAYER1_PIECE) or winning_move(board, AI_PIECE) or len(get_valid_locations(board)) == 0
				...
				if event.type == pygame.MOUSEBUTTONDOWN:
					...
					if turn == 0:
						...
						if is_valid_location(board, col) == False:
							...
							dropPiece(board, row, col, PLAYER1_PIECE)

							if winningMove(board, PLAYER1_PIECE):
								label = myfont.render("Player 1 wins!!", 1, RED)
								screen.blit(label, (40,10))
								game_over = True
					...
					# # Ask for Player 2 Input
						...
						if is_valid_location(board, col) == False:
							...
							row = get_NextOpenRow(board, col)
							dropPiece(board, row, col, PLAYER2_PIECE)
